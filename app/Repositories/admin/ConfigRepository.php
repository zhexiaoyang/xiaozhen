...
class ConfigRepository
{
    ...
    public function topConfigSelect($pid = 0)
    {
        $data = Config::statusEq1()->get()->toArray();
    ...
    {
        $menus = Config::status1()->select(['id','pid','name','display_name','icon'])->where('is_menu',1)->orderBy('sort','desc')->get()->toArray();
    ...
    public function getNodeId($name)
    {
        ...
        if ($name)
        {
            $node_id = Config::select('id','pid')->where('name',$name)->first()->toArray();
        }
        return $node_id;
    }
    ...
    {
        $permission = Config::withoutGlobalScope('status')->where('status','>=',0)->find($id);
        return $permission;
    }

    public function update($request,$id)
    {
        $permission = Config::find($id);
        if ($permission) {
...
//        abort(404);
    ...
    {
        $permissions = Config::status1()->select(['id','pid','name','display_name','icon'])->orderBy('sort','desc')->get()->toArray();
